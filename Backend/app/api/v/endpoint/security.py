...
def _save_scan_result(db: Session, user_id: int, result: dict) -> None:
...
async def scan_code(
    ...
    if ext in _ALLOWED_EXTENSIONS or ext == "":
        ...
        _save_scan_result(db, current_user.id, result)
        return result
    ...
    if ext in _ARCHIVE_EXTENSIONS:
        ...
        if sources:
            ...
            _save_scan_result(db, current_user.id, aggregated)
            return aggregated
